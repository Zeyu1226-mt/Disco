...
import Stage_two.models.LLM_models_dataset2_disentanglement_final as models
from util.utils import *
from util.optimization import AdamW
from LLM_disentanglement_dataset2 import LLM_dataset
import pickle
...
parser.add_argument('--video_feature_root', default='/home/xzy/xzy_nba/VG_TLD_Timesformer-features', type=str, help='npy features root')
parser.add_argument('--tokenizer_name', default="/media/nvme/XZY/LLM/meta-llama/Llama-3.2-3B", type=str, help='tokenizer name')
parser.add_argument('--full_top3_path', default="/media/nvme/XZY/Dataset/VG_2024_TLD_players", type=str, help='full pkl file root')
...
# train
parser.add_argument('--train_info_root', default="/home/xzy/xzy_nba/MLLM_VC/Stage_two/DATASET2/train_video_info.json", type=str, help='train json file root')
...
# test
parser.add_argument('--test_info_root', default="/home/xzy/xzy_nba/MLLM_VC/Stage_two/DATASET2/test_video_info.json", type=str, help='test json file root')
...
parser.add_argument('--save_root', default='/media/nvme/XZY/Results/Final_dataset2_new_lizi', type=str, help='save root')
...
# num_frame: 16, 20, 25
parser.add_argument('--num_frame', default=30, type=int, help='number of frames for each clip')
parser.add_argument('--max_words', default=30, type=int, help='number of frames for each clip')
...
# for i, (images, activities) in enumerate(tqdm(train_loader, total=7624, position=0)):
    for i, (video, caption_tokens, labels, attention_mask,
            ...
            gt_caption) in enumerate(tqdm(train_loader, total=8668, position=0)):
        # video = video.cuda()
...
def validate(test_loader, model, epoch):
    ...
    for i, (video, caption_tokens, labels, attention_mask,
            ...
            action_one_embeds, action_one_feature,
            action_two_embeds, action_two_feature,
            gt_caption) in enumerate(tqdm(test_loader, total=1060, position=0)):
        # video = video.cuda()
        # video_mask = video_mask.cuda()
        # caption_tokens = caption_tokens.cuda()
        # labels = labels.cuda()
        ...
        res_text = model(video, caption_tokens, labels, attention_mask,
                     ...
                     name_two_embeds, entity_two_feature,
                     action_one_embeds, action_one_feature,
                     action_two_embeds, action_two_feature, validating=True)
