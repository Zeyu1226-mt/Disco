...
class STGLayer(nn.Module):
    def __init__(self, hidden_dim=768, num_heads=12, mlp_ratio=4.0, dropout=0.,
        ...
        if not self.is_temporal_first:  # STG
            if not self.no_spatial:
                ...
                if not self.use_spatial_attn_rope:
                    ...
                    self.spatial_attn = MultiheadSelfAttentionWithRope(dim=hidden_dim, num_heads=num_heads,
                                                                       dropout=dropout,
            ...
            if not self.no_temporal:
                ...
                if not self.use_temporal_attn_rope:
                    ...
                    self.temporal_attn = MultiheadSelfAttentionWithRope(dim=hidden_dim, num_heads=num_heads,
                                                                        dropout=dropout,
                                                                        use_qkv_bias=use_rope_qkv_bias,
                                                                        use_qk_norm=use_rope_qkv_norm,
                                                                        use_RMSNorm=use_RMSNorm
        ...
        if not self.no_mlp:
            ...
            if self.use_mlp_swiglu:
                self.mlp = MLPSwiGLU(hidden_dim=hidden_dim, mlp_ratio=mlp_ratio, dropout=dropout,
                                     use_subln=use_mlp_swiglu_subln)
            else:
    ...
    def forward(self, x):
        ...
        if not self.is_temporal_first:
            # Spatial attention  is_temporal_first == False
...
class STG(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_pos_patch_level or self.use_pos_frame_level:
            if self.use_pos_patch_level:
                assert x.shape[2] == self.pos_embedding.shape[
                    2], f"Number of patch tokens should be equal to the number of position embeddings. {x.shape[2]} != {self.pos_embedding.shape[2]}"
