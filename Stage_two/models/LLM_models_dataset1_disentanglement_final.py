...
class LLM_Captioner(nn.Module):
    def __init__(self,
        ...
        file_path = '/home/xzy/xzy_nba/LLM_VC/Player_identify/code/B_data_preprocessing/C_dataset1_llama3-2-3B.pkl'
    ...
    def forward(self, video, caption_tokens, labels, attention_mask,
        ...
        # print("name_one_embeds: ", name_one_embeds.shape)  # name_one_embeds:  torch.Size([B, 8, 3072])
        # print("entity_one_feature: ", entity_one_feature.shape)  # entity_one_feature:  torch.Size([B, 1, 3920, 768])
        # print("action_one_embeds", action_one_embeds.shape)  # action_one_embeds torch.Size([B, 4, 3072])
        # print("action_one_feature: ", action_one_feature.shape)  # action_one_feature:  torch.Size([B, 1, 3920, 768])
        ...
        video_hidden = video_query_output.last_hidden_state  # print("video_hidden: ", video_hidden.shape)  # torch.Size([B, Q, 768])
        scene_feature = video_hidden  # B, Q, 768
        scene_feature = scene_feature.permute(1, 0, 2).contiguous()  # Q, B, 768

        #####
        inputs_llama = self.llama_proj(video_hidden)  # torch.Size([B, Q, 3072])

        # VE module
        # names_fusion = names_fusion.permute(1, 0, 2).contiguous()  # print(names_fusion.shape)  # num b 768
        #
        # q = k = v = names_fusion
        # name_att, _ = self.self_attn(q, k, v)
        ...
        entity_three_feature = einops.rearrange(entity_three_feature, 'b w n f -> (b w) n f', b=batch_size, n=3920, w=1)
        entity_three_feature = entity_three_feature.permute(1, 0, 2).contiguous().to(self.device) # 3920-B-768
        entity_three_feature, _ = self.self_attn1(entity_three_feature, scene_feature, scene_feature)
        entity_three_feature = entity_three_feature.permute(1, 0, 2).contiguous()
        entity_three_feature = torch.mean(entity_three_feature, dim=1).unsqueeze(1)
        ...
        action_three_feature = einops.rearrange(action_three_feature, 'b w n f -> (b w) n f', b=batch_size, n=3920, w=1)
        action_three_feature = action_three_feature.permute(1, 0, 2).contiguous().to(self.device)  # 3920-B-768
        action_three_feature, _ = self.self_attn2(action_three_feature, scene_feature, scene_feature)
        action_three_feature = action_three_feature.permute(1, 0, 2).contiguous()
        action_three_feature = torch.mean(action_three_feature, dim=1).unsqueeze(1)
        ...
        input_entity_llama1 = self.entity_out_proj(entity_one_feature)
        # print("input_entity_llama1", input_entity_llama1.shape)
        input_entity_llama2 = self.entity_out_proj(entity_two_feature)
        # print("input_entity_llama2", input_entity_llama2.shape)
        input_entity_llama3 = self.entity_out_proj(entity_three_feature)
        # print("input_entity_llama3", input_entity_llama3.shape)

        input_action_llama1 = self.action_out_proj(action_one_feature)
        # print("input_action_llama1", input_action_llama1.shape)
        input_action_llama2 = self.action_out_proj(action_two_feature)
        # print("input_action_llama2", input_action_llama2.shape)
        input_action_llama3 = self.action_out_proj(action_three_feature)
        # print("input_action_llama3", input_action_llama3.shape)
        ...
        if validating:
            return self.generate_text(inputs_llama,
                                      ...
                                      name_three_embeds, input_entity_llama3,
                                      action_one_embeds, input_action_llama1,
                                      action_two_embeds, input_action_llama2,
                                      action_three_embeds, input_action_llama3
                                      )
        ...
        # print("type", targets.dtype)  # 2个1*768 2个1*768 2个8 2个4
        visual_label = torch.full((batch_size, self.num_video_query_token + 3 + 3 + 24 + 12), -100, dtype=targets.dtype)
        ...
        name_three_embeds = name_three_embeds.view(batch_size, 8, -1).to(self.device)
        ...
        action_three_embeds = action_three_embeds.view(batch_size, 4, -1).to(self.device)
        ...
        mask_prefix = torch.ones(batch_size, self.num_video_query_token + 3 + 3 + 24 + 12, dtype=atts_llama.dtype)  #
    ...
    def generate_text(self, inputs_llama,
        ...
        mask_prefix = torch.ones(batch_size, self.num_video_query_token + 1 + 24 + 12 + 3 + 3, dtype=torch.bool).to(
            self.device)
