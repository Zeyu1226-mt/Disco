...
def pad_tensor_action(tensor, target_shape=(4,3072)):  # llama3.2-3B: 3072
    ...
    if current_shape [0] < target_shape[0]:
        ...
        tensor = torch.cat([tensor, padding_tensor], dim=0)
    return tensor
def sample_frames(features, target_frames=24):
    T = features.shape[0]  # 原始帧数（72）
    indices = torch.linspace(0, T - 1, target_frames).long()  # 均匀采样
    sampled_features = features[indices]  # 选取对应帧
...
class LLM_dataset(Dataset):
    def __init__(self, feature_root, video_info_path, full_top3_path, max_frames, max_words,
        ...
        # self.video_top3_pkl = pickle.load(open(self.videoid_top3, 'rb'))    # video102694 : {"xxx": array([[xxx]])}
    ...
    def __getitem__(self, index):
        ...
        v_name = str(video_id) + '.npy'
        video_feature = torch.from_numpy(np.load(os.path.join(self.feature_root, v_name)))
        T, _, _ = video_feature.shape
        # print("video_feature: ", video_feature.shape)  # torch.Size([16, 196, 768])
        video[0][:self.max_frames, :, :] = sample_frames(video_feature, self.max_frames)
        ...
        with torch.no_grad():
            ...
            try:
                ...
                entity_three_feature = entity_one_feature
                name_three = name_one
            ...
            try:
                ...
                action_three_feature = action_one_feature
                action_three = action_one
        ...
        # 补0直到长度达到了30
        while len(caption_tokens) < 35:  # 补0直到长度达到了30
        ...
        return (video, caption_tokens.detach(), labels.detach(), attention_mask.detach(),
                ...
                name_three.clone().detach(), entity_three_feature,
                action_one.detach(), action_one_feature,
                action_two.detach(), action_two_feature,
                action_three.detach(), action_three_feature,
...
class LLM_dataset_test(Dataset):
    def __init__(self, name_file, feature_root, video_info_path, full_top3_path, max_frames, max_words,
        ...
        self.video_info_dict = json.load(open(self.video_info, 'r'))
    ...
    def __getitem__(self, index):
        ...
        name_list = self.name_info[video_id]['gt_name']
